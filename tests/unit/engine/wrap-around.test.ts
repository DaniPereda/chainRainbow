...
describe('wrap-around: a piece pushed past the edge reappears on the opposite side (FR-001)', () => {
  ...
  it('reappears on the opposite edge and settles when that cell is empty', () => {
    ...
    expect(outcome.board.cells[2][7]).toEqual({ color: 'green', fragility: 'new' }); // the launcher survives and settles here (FR-007)
    expect(outcome.board.cells[2][0]).toEqual({ color: 'orange', fragility: 'cracked' }); // reappeared here
